...
def Introduce():
    ...
    with streamlit.expander("展开查看模型性能图"):
        ...
        col4.image("ImageLib/ModelDisplay_5.png", 
                    channels = "RGB", 
                    use_container_width = True)
    with open("TextLib/Introduce_menber.txt", "r", encoding = 'utf-8') as m:
        MenberText = m.readlines()
    for line in MenberText:
        streamlit.markdown(line)
...
def CodeDisplay():
    ...
    with streamlit.expander("💻模型部署\tLPRS_ModelWeb.py"):
        streamlit.code(body = CodeToStr("LPRS_Web.py"), language = "python", line_numbers = True)
...
def main():
    ...
    streamlit.write("学院: 职业技术师范学院")
    streamlit.write("班级: 22计科师资2班")
    streamlit.write("作者: 范嘉俊、赵宣朗、江淑雅、汪思琦")
